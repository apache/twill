 ...
 */
final class ModernKafkaConsumer implements org.apache.twill.kafka.client.KafkaConsumer {

  private static final Logger LOG = LoggerFactory.getLogger(ModernKafkaConsumer.class);
  ...
  ModernKafkaConsumer(String bootstrapServers) {
    this.brokerService = bootstrapServers;
   ...
   */
  private CacheLoader<Properties, KafkaConsumer<Integer, ByteBuffer>> createConsumerLoader() {
    return new CacheLoader<Properties, KafkaConsumer<Integer, ByteBuffer>() {
      @Override
      public KafkaConsumer<Integer, ByteBuffer> load(Properties props) throws Exception {
        return new KafkaConsumer<Integer, ByteBuffer>(props); //props.getHost(), props.getPort(), SO_TIMEOUT, FETCH_SIZE, "simple-kafka-client"
      }
    };
  }
   ...
   */
  private RemovalListener<Properties, KafkaConsumer<Integer, ByteBuffer>> createRemovalListener() {
    return new RemovalListener<Properties, KafkaConsumer<Integer, ByteBuffer>>() {
      @Override
      public void onRemoval(RemovalNotification<Properties, KafkaConsumer<Integer, ByteBuffer>> notification) {
        KafkaConsumer<Integer, ByteBuffer> consumer = notification.getValue();
        if (consumer != null) {
          consumer.close();
        }
      }
